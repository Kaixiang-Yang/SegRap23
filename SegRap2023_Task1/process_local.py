...
from Task001_postprocess import post_main
...
class Customalgorithm():  # SegmentationAlgorithm is not inherited in this class anymore
    def __init__(self):
        ...
        """
        self.nii_path = 'Task001_SegRap2023/imagesTs'
        self.result_path = 'Task001_SegRap2023/result'
        self.nii_seg_file = None#'SegRap2023_001.nii.gz'
        self.weight = "./weight/"
    ...
    def load_inputs(self):      # use two modalities input data
        ...
        """
        ct_mhas = sorted(os.listdir(os.path.join(self.input_dir, 'images/head-neck-ct/')))
        ctc_mhas = sorted(os.listdir(os.path.join(self.input_dir, 'images/head-neck-contrast-enhanced-ct/')))
        uuid = sorted(os.path.splitext(ct_mha)[0] for ct_mha in ct_mhas)
        ...
        """
        for num, (ct_mha, ctc_mha) in enumerate(zip(ct_mhas,ctc_mhas), start=120):
            self.convert_mha_to_nii(os.path.join(self.input_dir, 'images/head-neck-ct/', ct_mha),
                                    os.path.join(self.nii_path, f"SegRap2023_{num:03d}_0000.nii.gz"))
            self.convert_mha_to_nii(os.path.join(self.input_dir, 'images/head-neck-contrast-enhanced-ct/', ctc_mha),
                                    os.path.join(self.nii_path, f"SegRap2023_{num:03d}_0001.nii.gz"))
        
        # Check the validation/test data exist.
        print(os.listdir('Task001_SegRap2023/imagesTs'))
    ...
    def write_outputs(self, uuids):
        ...
        for uuid, nii in zip(uuids, self.nii_seg_file):
            convert_one_hot_label_to_multi_organs(os.path.join(
                self.result_path, nii), os.path.join(self.output_dir, uuid + ".mha"))
    ...
    def predict(self):
        ...
        """
        predict_from_folder_segrap2023(self.weight, self.nii_path, self.result_path, 'all', 0, 1)
        print("nnUNet segmentation done!")

        self.nii_seg_file = sorted(list(set([kk[:14]+'.nii.gz' for kk in os.listdir(self.nii_path)])))

        if not all(os.path.exists(os.path.join(self.result_path, nii)) for nii in self.nii_seg_file):
            print('waiting for nnUNet segmentation to be created')

        while not all(os.path.exists(os.path.join(self.result_path, nii)) for nii in self.nii_seg_file):
            import time
            print('.', end='')
            time.sleep(5)
    ...
    def post_process(self):
        ...
        uuids = self.load_inputs()
        print('Start prediction')
        self.predict()
        print('Start postpro for confusion')
        post_main(self.result_path, self.result_path, '', connectivity=6, mode=2, threshold=200, label=False)
        print('Start output writing')
        self.write_outputs(uuids)
