...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            #########################################
            #properties['classes']中从pkl中加载的信息含有"-1", 因此在此处进行相应处理，否则后续所有code都需要进行相应判定
            classes = np.array(properties['classes'])
            classes[classes < 0] = 0
            classes = np.unique(classes)
            class_for_batch.append(classes)
            #########################################
            ...
            if not force_fg:
                ...
                if voxels_of_that_class is not None:
                    ...
                    # selected_voxel = [mid//2 for mid in shape]#以中心为patch中心20221110
        ...
        ########################################################
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'class_for_batch': class_for_batch}
        ########################################################
