...
class CotLayer(nn.Module):
    ...
    def __init__(self, dim=512, kernel_size=3):
        ...
        self.se = nn.Sequential(
            nn.Conv3d(dim, max(dim*3//4,48), 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(max(dim*3//4,48), 3*dim, 1)
        )

    #diff: 1.relu inplace 2.att softmax
    #use the original feature and k1, k2 to obtain the se feature maps
    def forward(self, x):
        bs,c,d,h,w=x.shape
        k1=self.key_embed(x) #bs,c,d,h,w
        ...
        x0 = x.view(B,C,1,D,H,W)
        Y = torch.cat([k2,k1,x0], dim=2)#B,C,2,D,H,W

        x_gap = Y.sum(dim=2)
        x_gap = x_gap.mean((2,3,4), keepdim=True) #b,c,1,1,1
        x_attn = self.se(x_gap)#b,3*c,1,1,1
        x_attn = x_attn.view(B, C, 3)
        x_attn = F.softmax(x_attn, dim=2)
        out = (Y * x_attn.reshape(B,C,3,1,1,1)).sum(dim=2)
        # out += x
