...
class SegmentationNetwork(NeuralNetwork):
    ...
    @staticmethod
    def _old_compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        ...
        # for z, we use [1:-1] remove the first and the last slide window
        for dim in range(len(patch_size)):
            # the highest step value for this dimension is
            max_step_value = image_size[dim] - patch_size[dim]
            if num_steps[dim] > 1:
            ...
            steps.append(steps_here)
            # steps.append(steps_here[1:-1] if len(steps_here) > 2 else steps_here)
            # break
        
        # # for y, we consider the range [80:300] of imagesize
        # num_steps[1] = int(np.ceil((300-80-patch_size[1])/target_step_sizes_in_voxels[1])) + 1
        # actual_step_size = (300-80-patch_size[1])/(num_steps[1]-1) if num_steps[1] > 1 else 999999999999
        # steps_here = [int(np.round(actual_step_size * i)) + 80 for i in range(num_steps[1])]
        # steps.append(steps_here)

        # # for x, we consider the range [250:700] of imagesize
        # num_steps[2] = int(np.ceil((700-250-patch_size[2])/target_step_sizes_in_voxels[2])) + 1
        # actual_step_size = (700-250-patch_size[2])/(num_steps[2]-1) if num_steps[2] > 1 else 999999999999
        # steps_here = [int(np.round(actual_step_size * i)) + 250 for i in range(num_steps[2])]
        # steps.append(steps_here)

        # # for y, we consider [img//2-patch*stepsize]
        # steps_here = [int(np.round(image_size[1]/2)-0.5*patch_size[1])]
        # steps.append(steps_here)

        # # for x, we consider [img//2-patch,img//2-patch*stepsize,img//2+patch//2]
